// StarkLabGestureCommand.jsx
// design by prashant

import React, { useEffect, useRef } from "react";
import * as THREE from "three";

// THREE.JS POST-PROCESSING IMPORTS
import { EffectComposer } from "three/examples/jsm/postprocessing/EffectComposer.js";
import { RenderPass } from "three/examples/jsm/postprocessing/RenderPass.js";
import { UnrealBloomPass } from "three/examples/jsm/postprocessing/UnrealBloomPass.js";

// MEDIAPIPE IMPORTS (VITE COMPATIBLE)
...
const StarkLabGestureCommand = () => {
  ...
  useEffect(() => {
    ...
    const CONFIG = {
      ...
      bloomStrength: isMobile ? 0.8 : 1.2,
      bloomRadius: 0.8,
      lerpSpeed: 0.03,
      fastLerpSpeed: 0.12,
      transitionDuration: 1000,
      camSmooth: isMobile ? 0.05 : 0.08,
      zoomMin: 30,
      zoomMax: isMobile ? 70 : 90,
    };
    ...
    const shaderMaterial = new THREE.ShaderMaterial({
      uniforms: {
        ...
        size: { value: isMobile ? 2 * window.devicePixelRatio : 3.5 * window.devicePixelRatio },
      },
      vertexShader: `
        uniform float time;
        uniform float size;
    ...
    function updateFormation() {
      ...
      bloomPass.strength = isMobile ? 1.5 : 3.5;
      setTimeout(() => {
        bloomPass.strength = CONFIG.bloomStrength;
      }, 300);
    ...
    const handleGestures = (results) => {
      ...
      if (results.landmarks.length > 0) {
        ...
        if (results.landmarks.length === 1) {
          ...
          let normDist = Math.max(0, Math.min(1, (dist - 0.2) * 2.0));
    ...
    const animate = () => {
      ...
      const currentLerpSpeed = now < state.transitionEndTime ? CONFIG.fastLerpSpeed : CONFIG.lerpSpeed;
      ...
      if (state.hasHands) {
        ...
        state.targetRotX = 0;
        state.targetZoom = 60;
        state.currentRotX += (0 - state.currentRotX) * 0.01;
        state.currentRotY = state.autoRotateAngle * 0.2;
        state.currentZoom += (60 - state.currentZoom) * 0.01;
      }
    ...
    const handleResize = () => {
      ...
      shaderMaterial.uniforms.size.value = isMobile ? 2 * window.devicePixelRatio : 3.5 * window.devicePixelRatio;
    };
  ...
  return (
    <>
      <video
        ...
        playsInline
        muted
        style={{ position: "absolute", top: 0, left: 0, opacity: 0, pointerEvents: "none", zIndex: 0 }}
      />
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
          ...
          <span ref={lStatRef} style={{ fontWeight: "bold" }}>
            --
          </span>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
          <span style={{ opacity: 0.7 }}>R-HAND</span>
          <span ref={rStatRef} style={{ fontWeight: "bold" }}>
            --
          </span>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 0 }}>
          ...
          <span ref={cmdStatRef} style={{ fontWeight: "bold" }}>
            WAITING
          </span>
        </div>
      </div>
